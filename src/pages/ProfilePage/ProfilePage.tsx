...
const submitReview = async () => {
  ...
  try {
    ...
    const response = await axios.post<{ isFailed: boolean; message: string }>("http://localhost:5122/api/Customer/ReviewProduct", reviewPayload);
  ...
  const Star = ({ selected = false, onClick }) => (
    <span 
      onClick={onClick} 
      className={`text-2xl cursor-pointer ${selected ? 'text-yellow-600' : 'text-gray-300'}`}
    >
      ★
    </span>
  );
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div>
               ...
               <p><strong>{language === "en" ? "Member ID:" : "አባል መታወቂያ:"}</strong> {user?.id || 'N/A'}</p>
             </div>
             <div>
