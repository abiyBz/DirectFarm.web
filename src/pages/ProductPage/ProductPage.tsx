  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const response = await axios.post<Product>(`http://localhost:5122/api/Product/GetProduct`, { id });
        const productData = response.data; // Assuming a single product object is returned
    ...
    <div className="max-w-4xl mx-auto p-8 bg-white rounded-lg shadow-lg">
      <div className="flex flex-col lg:flex-row items-center">
        ...
        <div className="w-full lg:w-1/3 mb-8 lg:mb-0">
          <img
            className="w-full h-full object-cover rounded-lg shadow-md"
            src={product.image || 'path/to/placeholder/image.png'} // Use placeholder image if no image is available
            alt={product.name}
          />
        </div>
