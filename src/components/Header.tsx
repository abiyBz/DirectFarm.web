              ...
              <div className="flex justify-items-center items-center gap-2">
                ...
                <div style={{ position: 'relative', left: '30px' }}>
                  <input 
                    ...
                    placeholder={language === "en" ? "Search for Different Fruits, Vegetable, or Meat" : "የተለያዩ ፍራፍሬ፣ አትክልት ወይም ስጋ ይፈልጉ"} 
                    value={searchTerm}
                  ...
                  {filteredProducts.length > 0 && (
                    <div ref={dropdownRef} className="absolute z-10 w-[350px] bg-white border border-gray-300 rounded-md mt-2 shadow-lg">
                      {filteredProducts.map((product) => (
                        
                        <Link key={product.id} to={`/product/${product.id}`} className="flex items-center py-4 px-8 hover:bg-gray-100 cursor-pointer block">
                          <img src={product.image} alt={product.name} className="w-8 h-8 mr-2 object-cover" /> {/* Adjust size as needed */}
                          <span className="text-black">{product.nameAmharic}/{product.name}</span>
                        </Link>
                      ))}
                    </div>
                  )}
