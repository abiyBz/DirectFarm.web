...
const Inventory: React.FC = () => {
  ...
  const [user, setUser] = useState<string>(""); // Use lowercase 'string'
    ...
    if (storedLoginResponse) {
      try {
        ...
        if (managerId) {
          ...
          console.error("Manager ID not found in the parsed response");
          setError("Could not retrieve manager ID");
        }
      } catch (error) {
        console.error("Failed to parse login response:", error);
  ...
  useEffect(() => {
    const fetchFarmers = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Network response was not ok for farmers");
        }
        ...
        setLoading(prev => prev && false); // Only set to false if it was true
      }
    };
  ...
  useEffect(() => {
    const fetchWarehouses = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error("Network response was not ok for warehouses");
        }
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    if (!selectedFarmer || !selectedProduct || !selectedWarehouse || productQuantity <= 0) {
  ...
  return (
    <div className="max-w-xl mx-auto p-8 text-black">
      ...
      <form onSubmit={handleAddProduct} className="space-y-6 mb-8">
        <div>
          <label htmlFor="products" className="block text-lg font-semibold">
            Select a product
          </label>
          <select
            id="products"
        ...
        <div>
          <label htmlFor="farmers" className="block text-lg font-semibold">
            Select Farmer
          </label>
          <select
            id="farmers"
        ...
        <div>
          <label htmlFor="warehouses" className="block text-lg font-semibold">
            Select a warehouse
          </label>
          <select
            id="warehouses"
